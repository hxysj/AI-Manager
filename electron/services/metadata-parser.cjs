...
class MetadataParser {
  async parse(skillRoot, repoId = null) {
    ...
    const iconCandidate = metadata.icon ? path.join(skillRoot, String(metadata.icon).trim()) : null
    const fallbackIcon = path.join(skillRoot, 'icon.png')
    const iconPath = (iconCandidate && (await fileExists(iconCandidate)))
      ? iconCandidate
    ...
    return {
      id: crypto.createHash('sha1').update(skillRoot).digest('hex').slice(0, 16),
      name: skillName,
